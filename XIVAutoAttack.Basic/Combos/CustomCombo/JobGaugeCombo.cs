...
{
    public abstract class JobGaugeCombo<T, TCmd> : CustomCombo<TCmd> where T : JobGaugeBase where TCmd : Enum
    {
        private static T _gauge;

        internal static T JobGauge
        {
            get
            {
                if (_gauge == null)
        ...
        protected bool UseBreakItem(out IAction act)
        {
            ...
            Role role = (Role)CommandController.AllJobs.First(job => job.RowId == Service.ClientState.LocalPlayer.ClassJob.Id).Role;
            switch (role)
            {
                case Role.近战:
                case Role.防护:
