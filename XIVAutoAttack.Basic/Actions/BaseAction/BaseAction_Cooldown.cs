...
namespace XIVAutoAttack.Actions.BaseAction
{
    public partial class BaseAction
    {
        ...
        public bool ElapsedAfterGCD(uint gcdCount = 0, uint abilityCount = 0, bool addWeaponElapsed = true)
        {
        ...
        /// <param name="addWeaponElapsed">是否要把<see cref="ActionUpdater.WeaponElapsed"/>加进去</param>
        /// <returns>是否已经冷却了这么久了</returns>
        public bool ElapsedAfter(float gcdelapsed, bool addWeaponElapsed = true)
        {
            if (!IsCoolDown) return false;
            var elapsed = RecastTimeElapsedOneCharge;
        ...
        public bool WillHaveOneChargeGCD(uint gcdCount = 0, uint abilityCount = 0, bool addWeaponRemain = true)
        {
        ...
        /// <param name="addWeaponRemain">是否要把<see cref="ActionUpdater.WeaponRemain"/>加进去</param>
        /// <returns>这个时间点是否起码有一层可以用</returns>
        public bool WillHaveOneCharge(float remain, bool addWeaponRemain = true)
        {
            if (HaveOneCharge) return true;
            var recast = RecastTimeRemainOneCharge;
        ...
        public ushort ChargesCount => IsCoolDown ? (ushort)(RecastTimeElapsed / RecastTimeOneCharge) : MaxCharges;
