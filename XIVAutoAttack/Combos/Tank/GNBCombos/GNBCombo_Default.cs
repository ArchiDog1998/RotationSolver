...
internal sealed class GNBCombo_Default : GNBCombo_Base<CommandType>
{
    ...
    private protected override bool AttackAbility(byte abilityRemain, out IAction act)
    {
        ...
        if (DangerZone.ShouldUse(out act))
        {
            ...
            //等级小于烈牙,
            if (!GnashingFang.EnoughLevel && (Player.HaveStatus(true, StatusID.NoMercy) || !NoMercy.WillHaveOneCharge(15, false))) return true;

            //爆发期,烈牙之后
            if (Player.HaveStatus(true, StatusID.NoMercy) && GnashingFang.IsCoolDown) return true;

            //非爆发期
            if (!Player.HaveStatus(true, StatusID.NoMercy) && !GnashingFang.WillHaveOneCharge(20, false)) return true;
        }
        ...
        if (RoughDivide.Target.DistanceToPlayer() < 1 && !IsMoving)
        {
            ...
            if (Player.HaveStatus(true, StatusID.NoMercy) && RoughDivide.ShouldUse(out act, emptyOrSkipCombo: true)) return true;
        }
        act = null;
        return false;
    }
    ...
    private bool CanUseGnashingFang(out IAction act)
    {
        ...
        if (GnashingFang.ShouldUse(out act))
        {
            ...
            //无情中3弹烈牙
            if (JobGauge.Ammo == (Level >= 88 ? 3 : 2) && (Player.HaveStatus(true, StatusID.NoMercy) || !NoMercy.WillHaveOneCharge(55, false))) return true;
    ...
    private bool CanUseSonicBreak(out IAction act)
    {
        ...
        if (SonicBreak.ShouldUse(out act))
        {
            ...
            if (!GnashingFang.EnoughLevel && Player.HaveStatus(true, StatusID.NoMercy)) return true;

            //在烈牙后面使用音速破
            if (GnashingFang.IsCoolDown && Player.HaveStatus(true, StatusID.NoMercy)) return true;

            //其他判断
            if (!DoubleDown.EnoughLevel && Player.HaveStatus(true, StatusID.ReadyToRip)
    ...
    private bool CanUseDoubleDown(out IAction act)
    {
        ...
        if (DoubleDown.ShouldUse(out act, mustUse: true))
        {
            ...
            if (InDungeonsMiddle)
            {
                ...
                if (Player.HaveStatus(true, StatusID.NoMercy)) return true;

                return false;
            }

            //在音速破后使用倍攻
            if (SonicBreak.IsCoolDown && Player.HaveStatus(true, StatusID.NoMercy)) return true;

            //2弹无情的特殊判断,提前使用倍攻
            if (Player.HaveStatus(true, StatusID.NoMercy) && !NoMercy.WillHaveOneCharge(55, false) && Bloodfest.WillHaveOneCharge(5, false)) return true;
    ...
    private bool CanUseBurstStrike(out IAction act)
    {
        if (BurstStrike.ShouldUse(out act))
        {
            ...
            //无情中爆发击判定
            if (Player.HaveStatus(true, StatusID.NoMercy) &&
    ...
    private bool CanUseBowShock(out IAction act)
    {
        if (BowShock.ShouldUse(out act, mustUse: true))
        {
            ...
            if (!SonicBreak.EnoughLevel && Player.HaveStatus(true, StatusID.NoMercy)) return true;

            //爆发期,无情中且音速破在冷却中
            if (Player.HaveStatus(true, StatusID.NoMercy) && SonicBreak.IsCoolDown) return true;
