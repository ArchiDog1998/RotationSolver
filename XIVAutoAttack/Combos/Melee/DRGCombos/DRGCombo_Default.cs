...
internal sealed class DRGCombo_Default : DRGCombo_Base<CommandType>
{
    ...
    private protected override bool EmergercyAbility(byte abilityRemain, IAction nextGCD, out IAction act)
    {
        if (nextGCD.IsAnySameAction(true, FullThrust, CoerthanTorment)
            || Player.HaveStatus(StatusID.LanceCharge) && nextGCD.IsAnySameAction(false, FangandClaw))
        {
            //龙剑
    ...
    private protected override bool AttackAbility(byte abilityRemain, out IAction act)
    {
        if (SettingBreak)
        {
            ...
            {
                if (abilityRemain == 1 && !Player.HaveStatus(StatusID.PowerSurge)) return true;
                if (Player.HaveStatus(StatusID.PowerSurge)) return true;
            }
        ...
        {
            if (Player.HaveStatus(StatusID.LanceCharge) && LanceCharge.ElapsedAfterGCD(3)) return true;
        }
        if (Player.HaveStatus(StatusID.PowerSurge) && SpineshatterDive.ChargesCount != 1 && SpineshatterDive.ShouldUse(out act)) return true;
