...
internal sealed class MCHCombo_Default : MCHCombo_Base<CommandType>
{
    ...
    private protected override bool GeneralGCD(out IAction act)
    {
        ...
        if (!JobGauge.IsOverheated || JobGauge.IsOverheated && EndAfterGCD(JobGauge.OverheatTimeRemaining, 0))
        {
            ...
            {
                if (Player.HaveStatus(true, StatusID.Reassemble)) return true;
    ...
    {
        if (!Hypercharge.ShouldUse(out act) || Player.HaveStatus(true, StatusID.Reassemble)) return false;

        //有野火buff必须释放超荷
        if (Player.HaveStatus(true, StatusID.Wildfire)) return true;
