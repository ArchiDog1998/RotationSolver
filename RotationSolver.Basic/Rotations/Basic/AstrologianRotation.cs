...
partial class AstrologianRotation
{
    ...
    private sealed protected override IBaseAction Raise => Ascend_PvE;
    private protected override IBaseAction LimitBreak => AstralStasis_PvE;
    ...
    protected bool PlayCard(out IAction act)
    {
        ...
        if (TheBalance_PvE.CanUse(out act, CanUseOption.OnLastAbility)) return true;
        if (TheArrow_PvE.CanUse(out act, CanUseOption.OnLastAbility)) return true;
        if (TheSpear_PvE.CanUse(out act, CanUseOption.OnLastAbility)) return true;
        if (TheBole_PvE.CanUse(out act, CanUseOption.OnLastAbility)) return true;
        if (TheEwer_PvE.CanUse(out act, CanUseOption.OnLastAbility)) return true;
        if (TheSpire_PvE.CanUse(out act, CanUseOption.OnLastAbility)) return true;
    ...
    public override void DisplayStatus()
    {
        ...
        ImGui.Text($"Redraw: {Redraw_PvE.ActionCheck(null, false)}");
    }
}
