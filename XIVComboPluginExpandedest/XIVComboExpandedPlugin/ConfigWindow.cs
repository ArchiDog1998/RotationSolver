...
internal class ConfigWindow : Window
{
	...
	public ConfigWindow()
		: base("Custom Combo Setup", (ImGuiWindowFlags)0, false)
	{
		this.RespectCloseHotkey = true;
		groupedPresets = (from preset in Enum.GetValues<CustomComboPreset>()
			select (preset, EnumExtensions.GetAttribute<CustomComboInfoAttribute>((Enum)preset)) into tpl
			where tpl.Item2 != null
			orderby tpl.Item2.JobName, tpl.Item2.Order
		...
		this.SizeCondition = (ImGuiCond)4;
		this.Size = new Vector2(740f, 490f);
	}
			...
			if (ImGui.CollapsingHeader(key))
			{
				foreach (var item4 in groupedPresets[key])
				{
					...
					if (flag2)
					{
						...
						ImGui.Text(FontAwesomeExtensions.ToIconString((FontAwesomeIcon)61445));
						ImGui.PopStyleColor();
						ImGui.PopFont();
						if (ImGui.IsItemHovered())
						{
					...
					if (parent.HasValue)
					{
						CustomComboInfoAttribute attribute = EnumExtensions.GetAttribute<CustomComboInfoAttribute>((Enum)item);
					...
					if (conflicts.Length != 0)
					{
						...
						{
							CustomComboInfoAttribute attribute2 = EnumExtensions.GetAttribute<CustomComboInfoAttribute>((Enum)preset);
							return "\n - " + attribute2.FancyName;
						}).Aggregate((string t1, string t2) => t1 + t2);
