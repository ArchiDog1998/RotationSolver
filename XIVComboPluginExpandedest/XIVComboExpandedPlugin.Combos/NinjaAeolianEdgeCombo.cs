...
internal class NinjaAeolianEdgeCombo : CustomCombo
{
	...
	protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
	{
		...
		if (actionID == 2255)
		{
			if (CustomCombo.IsEnabled(CustomComboPreset.NinjaBunshinFeature) && level >= 82)
			{
		...
		return actionID;
		IL_004d:
		if (((uint)num2 | ((CustomCombo.TargetHasEffect(638) && CustomCombo.HasEffect(2723)) ? 1u : 0u)) != 0)
		{
			return 25774u;
		}
		goto IL_006f;
		...
		if (CustomCombo.IsEnabled(CustomComboPreset.NinjaArmorCrushRaijuFeature))
		{
			...
			{
				return CustomCombo.OriginalHook(25778u);
			}
		...
		if (CustomCombo.IsEnabled(CustomComboPreset.NinjaAeolianEdgeRaijuFeature))
		{
			if (level >= 90 && CustomCombo.HasEffect(2691))
			{
