...
internal abstract class CustomCombo
{
	...
	{
		CustomComboInfoAttribute attribute = EnumExtensions.GetAttribute<CustomComboInfoAttribute>((Enum)Preset);
	...
	public bool TryInvoke(uint actionID, uint lastComboActionID, float comboTime, byte level, out uint newActionID)
	{
		...
		uint? num = ((localPlayer != null) ? new uint?(((Character)localPlayer).ClassJob.Id) : null);
	...
	{
		return actions.Select(new Func<uint, (uint, IconReplacer.CooldownData)>(Selector)).Aggregate<(uint, IconReplacer.CooldownData)>(((uint ActionID, IconReplacer.CooldownData Data) a1, (uint ActionID, IconReplacer.CooldownData Data) a2) => Compare(original, a1, a2)).Item1;
	...
	protected static Status? FindEffect(ushort effectID)
	{
		...
		return FindEffect(effectID, (GameObject?)(object)localPlayer, (localPlayer2 != null) ? new uint?(((GameObject)localPlayer2).ObjectId) : null);
	}
	...
	protected static Status? FindTargetEffect(ushort effectID)
	{
		...
		PlayerCharacter? localPlayer = LocalPlayer;
		return FindEffect(effectID, currentTarget, (localPlayer != null) ? new uint?(((GameObject)localPlayer).ObjectId) : null);
	}
