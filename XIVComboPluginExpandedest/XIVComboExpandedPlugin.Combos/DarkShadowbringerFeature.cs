	...
	protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
	{
		if (actionID == 3643 || actionID == 3641)
		{
			if (level >= 90)
			{
				...
				obj[0] = actionID;
				return CustomCombo.CalcBestAction(actionID, obj);
			}
			if (level >= 86)
			{
				return CustomCombo.CalcBestAction(actionID, actionID, 3639u, 25755u);
			}
			if (level >= 56)
			{
				return CustomCombo.CalcBestAction(actionID, actionID, 3639u);
			}
			if (level >= 52)
			{
				return 3639u;
