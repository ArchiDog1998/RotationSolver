...
internal class ASTdotFeature : CustomCombo
{
	...
	protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
	{
		...
		{
			if (((TargetBuffDuration(1881) < 3f && level >= 72) || (TargetBuffDuration(843) < 3f && level >= 46 && level < 72) || (TargetBuffDuration(838) < 3f && level > 3 && level < 46)) && CustomCombo.HasCondition((ConditionFlag)26))
			{
				return CustomCombo.OriginalHook(3599u);
			}
			return CustomCombo.OriginalHook(3596u);
		}
		return actionID;
	}
}
