...
internal class WhiteStoneFeature : CustomCombo
{
	...
	protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
	{
		...
		{
			if (((TargetBuffDuration(1871) < 3f && level > 71) || (TargetBuffDuration(144) < 3f && level > 45 && level <= 71) || (TargetBuffDuration(143) < 3f && level > 3 && level < 46)) && CustomCombo.HasCondition((ConditionFlag)26))
			{
				return CustomCombo.OriginalHook(121u);
			}
			return CustomCombo.OriginalHook(119u);
		}
		return actionID;
	}
}
